...
namespace Microsoft.Azure.Commands.Attestation.Models
{
    internal class DataServiceCredential
    {
        ...
        public DataServiceCredential(IAuthenticationFactory authFactory, IAzureContext context,
            string resourceIdEndpoint)
        {
            if (authFactory == null)
        ...
        private static Tuple<IAccessToken, string> GetTokenInternal(string tenantId, IAuthenticationFactory authFactory, IAzureContext context, string resourceIdEndpoint)
        {
            if (string.IsNullOrWhiteSpace(tenantId))
                throw new ArgumentException("No Tenant In Context");
            ...
            {
                throw new ArgumentException("Invalid Subscription State", ex);
            }
        }

        /// <summary>
        /// Authentication callback method required by attestationClient
        /// </summary>
        ...
        public Task<string> OnAuthentication(string authority, string resource, string scope)
        {
            ...
            var bundle = GetTokenInternal(this.TenantId, this._authenticationFactory, this._context,
                this._endpointName);
            bundle.Item1.AuthorizeRequest((tokenType, tokenValue) => { tokenStr = tokenValue; });
